  ...
  return (
    <div className="relative">
      <button 
        ...
        className="relative w-10 h-10 rounded-full flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-slate-500 dark:text-slate-400 hover:text-primary"
      >
        <Bell size={20} />
        {unreadCount > 0 && (
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-80 glass-neu-card z-50 overflow-hidden">
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
            <h3 className="font-bold text-sm">Notifications</h3>
            {unreadCount > 0 && (
              <span className="text-[10px] font-bold text-primary dark:text-white bg-primary/10 px-2 py-0.5 rounded-full">{unreadCount} New</span>
            )}
          </div>
          <div className="max-h-96 overflow-y-auto">
            {notifications.map((n) => (
              <div key={n.id} className={`p-4 border-b last:border-0 border-slate-100 dark:border-slate-800 flex gap-3 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer transition-colors ${n.unread ? 'bg-primary/5' : ''}`}>
                <div className={`mt-0.5 shrink-0 ${n.type === 'invite' ? 'text-emerald-500' : n.type === 'mention' ? 'text-primary dark:text-white' : 'text-slate-400'}`}>
