      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm text-gray-500 dark:text-slate-400">• 2h ago</span>
          </div>
          <h3 className="text-lg font-bold mb-2">{thread.title}</h3>
          <p className="text-gray-700 dark:text-slate-200 leading-relaxed">{thread.content}</p>
        </div>
      </div>

      <div className="flex items-center gap-6 pt-4 border-t text-sm text-gray-500 dark:text-slate-400">
        ...
        <div className="pl-6 border-l-2 border-gray-100 space-y-6 mt-6">
          {thread.replies.map((reply: any) => (
            <div key={reply.id} className="space-y-2">
              ...
              <p className="text-sm text-gray-700 dark:text-slate-200">{reply.content}</p>
            </div>
          ))}
        </div>
      )}
