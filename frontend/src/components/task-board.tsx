    ...
    <div className="glass-neu-card p-6">
      <form onSubmit={addTask} className="flex gap-2 mb-6">
        <input
          ...
          className="flex-1 px-4 py-2.5 neu-inset focus:ring-2 focus:ring-primary outline-none text-sm"
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {COLUMNS.map((col) => {
          ...
          return (
            <div key={col.key} className="glass-neu-card p-3 min-h-[140px]">
              <div className="flex items-center gap-2 mb-3 px-1">
                <span className={`w-2 h-2 rounded-full ${col.accent}`} />
                <span className="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">{col.label}</span>
                <span className="ml-auto text-[10px] text-slate-400">{items.length}</span>
              </div>
              <div className="space-y-2">
                {items.map((task) => (
                  <div key={task.id} className="group glass-neu-card p-3">
                    ...
                    {task.status !== "DONE" && (
                      <button onClick={() => advance(task)} className="mt-2 text-[10px] font-bold text-primary dark:text-white hover:underline">
